...
public partial class tool_tool1307 : Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (UserInfo.Current(Session).UserRank < cons.comn.user.UserInfo.LEVEL_02)
        {
            Wrps.CallBack(Session, Request.RawUrl);
            Response.Redirect("~/user/user0101.aspx");
            return;
        }

        // Master Page初始化
        Session[cons.wrp.WrpCons.GUIDNAME] = "链接编辑";
        Session[cons.wrp.WrpCons.SCRIPTID] = "1307";
        ...
        ck_P3070104.Attributes.Add("onchange", "return changeView();");
    ...
    protected void bt_P3070106_Click(object sender, EventArgs e)
    {
        ...
        fileName = hd_P3070106.Value;
        if (!StringUtil.isValidate(fileName, 16))
        {
            fileName = HashUtil.getCurrTimeHex(false);
        }

        fu_P3070106.SaveAs(Server.MapPath(LinkCons.LINK_ICON_PATH) + fileName + ".png");
        im_P3070106.ImageUrl = LinkCons.LINK_ICON_PATH + fileName + ".png";
        hd_P3070106.Value = fileName;
    }
