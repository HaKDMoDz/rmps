...
{
    public partial class DImg : Form
    {
        #region 全局变量
        ...
        public void ShowImg(string file)
        {
            if (File.Exists(file))
            {
            ...
            int x = (PbImg.Width - _SrcImage.Width) >> 1;
            if (x < 0)
            {
                x = 0;
            }
            int y = (PbImg.Height - _SrcImage.Height) >> 1;
            if (y < 0)
            {
                y = 0;
            }
            PbImg.Location = new Point(x, y);
