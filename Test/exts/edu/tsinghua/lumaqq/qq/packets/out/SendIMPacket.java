...
public class SendIMPacket extends BasicOutPacket {
    ...
    protected void putBody(ByteBuffer buf) {
	    ...
	    // 接收者QQ号
	    buf.putInt(receiver);
		buf.putInt(0x0000000D );
		buf.putInt( 0x00010004 );
		buf.putInt( 0x00000000 );
		
		buf.putChar((char)0x0003); //未知长度
		buf.put(new byte[]{0x00,0x01,0x01}); //未知内容
...
//	    fontStyle.writeBean(buf);
		...
		buf.putInt(0x00000000 );
		buf.putInt(time);
		buf.putInt(messageId);	//maybe a random interger
		buf.putInt(0x00000000 );
		buf.putInt(0x09008600 );
		byte[] fontName = Util.getBytes("宋体", "UTF-8");	//must be UTF8
		buf.putChar((char)fontName.length );
		buf.put(fontName);
		buf.putChar((char)0x0000 );
		buf.put((byte)0x01 );
		buf.putChar((char)(message.length + 3) );
		buf.put((byte)0x1);
