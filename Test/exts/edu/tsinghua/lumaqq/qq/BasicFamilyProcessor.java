...
import com.amonsoft.util.HashUtil;
    ...
    private void processLoginReply(BasicInPacket in) {
    	...
    	switch(packet.replyCode) {
    		case QQ.QQ_REPLY_OK:
    		    ...
    		    user.setFileSessionKey(HashUtil.digest(b, "MD5"));
    		    // 触发事件 
...
//            	processGetLoginTokenReply(in);
            	processLoginRequestReply(in);
            	break;
            case QQ.QQ09_CMD_LOGIN_TOUCH:
            	this.user = client.getUser();
            	processLoginTouchReply(in);
            	break;
            case QQ.QQ09_CMD_LOGIN_VERIFY:
            	processLoginVerifyReply(in);
            	break;
            case QQ.QQ09_CMD_LOGIN_GET_INFO:
            	processLoginGetInfoReply(in);
            	break;
            case QQ.QQ09_CMD_LOGIN_SEND_INFO:
            	processLoginSendInfoReply(in);
            	break;
            case QQ.QQ09_CMD_LOGIN_GET_LIST:
            	processLoginGetListReply(in);
            	break;
    ...
    private void processLoginSendInfoReply(BasicInPacket in) {
    	QQEvent e;
		LoginSendInfoReplyPacket packet = (LoginSendInfoReplyPacket)in;
		switch(packet.replyCode) {
		case QQ.QQ_REPLY_OK:
	    	user.setSessionKey(packet.sessionKey);
	    	user.setIp(packet.clientIp);
	    	user.setPort(packet.clientPort);
	    	user.setLoginTime(packet.loginTime);
	    	user.setLastLoginTime(packet.lastLoginTime);
	...
	private void processLoginGetInfoReply(BasicInPacket in) {
    	QQEvent e;
		LoginGetInfoReplyPacket packet = (LoginGetInfoReplyPacket)in;
		//设置用户信息
    	user.setLoginInfoKey2(packet.loginInfoKey);
    	user.setLoginInfoUnknow2(packet.loginInfoUnknow);
    	user.setServerTime(packet.serverTime);
    	user.setLoginInfoLarge(packet.loginInfoLarge);
    	user.setIp(packet.clientIp);
    	user.getContactInfo().qq = user.getQQ();
    	user.getContactInfo().nick = packet.nickName;
    	
    	e = new QQEvent(packet);
		e.type = QQEvent.LOGIN_GET_INFO_OK;
		client.fireQQEvent(e);
	}
    

	private void processLoginVerifyReply(BasicInPacket in) {
		QQEvent e;
		LoginVerifyReplyPacket packet = (LoginVerifyReplyPacket)in;
		switch(packet.replyCode) {
		case QQ.QQ_REPLY_OK:
			user.setLoginToken(packet.loginToken);
    		user.setServerTime(packet.serverTime); 
    		user.setLoginInfoUnknow1(packet.loginInfoUnknow1);
    		user.setLoginInfoData(packet.loginInfoData);
    		user.setLoginInfoMagic(packet.loginInfoMagic);
    		user.setLoginInfoKey1(packet.loginInfoKey1);
    		user.setLoginInfoKey3(packet.loginInfoKey3);
			e = new QQEvent(packet);
			e.type = QQEvent.LOGIN_VERIFY_OK;
			client.fireQQEvent(e);
			break;
		case 0x33:
		case 0x51:	//登录拒绝
		case (byte)0xBF: //No this number.
		case 0x34: //Wrong password.
			e = new QQEvent(packet);
			e.type = QQEvent.LOGIN_FAIL;
			client.fireQQEvent(e);
			break;
	...
	private void processAuthInfoOpReply(BasicInPacket in) {
		...
		switch(packet.subCommand) {
			case QQ.QQ_SUB_CMD_GET_AUTH_INFO:
				...
				client.fireQQEvent(e);
				user.setAuthInfo(packet.authInfo);
				break;
    ...
    private void processLoginRequestReply(BasicInPacket in) {
        QQEvent e;
        LoginRequestReplyPacket packet = (LoginRequestReplyPacket)in;
        switch(packet.replyCode) {
        	case QQ.QQ_REPLY_OK:
        		if (packet.hasPngData == 1) {
        			if (packet.next == 1) { 
        				client.getPngDataBuf().put(packet.pngData);
        				client.loginRequest(packet.pngToken, null, packet.hasPngData);
        			} else {
        				ByteBuffer buf = client.getPngDataBuf();
        				buf.put(packet.pngData);
        				packet.pngData = buf.array();
        				buf.clear();
        			...
        			e = new QQEvent(packet);
        			user.setAuthToken(packet.answerToken);
                    e.type = QQEvent.LOGIN_REQUEST_OK;
                    client.fireQQEvent(e);
        		}
    ...
    private void processLoginTouchReply(BasicInPacket in) {
        QQEvent e;
        LoginTouchReplyPacket packet = (LoginTouchReplyPacket)in;
        switch(packet.replyCode) {
        	case QQ.QQ_REPLY_OK:
        		if (packet.testResult == 0) {
        			 user.setLoginToken(packet.loginToken);
        			 user.setServerIp(packet.serverIp);
                     e = new QQEvent(packet);
                     e.type = QQEvent.LOGIN_TOUCH_OK;
                     client.fireQQEvent(e);
        		} else {
        			log.debug("登陆重定向");
        			if(Util.isIpZero(packet.redirectIp)) {
...
//				break;
        ...
        if(packet.replyCode == QQ.QQ_REPLY_OK) {
        	switch(packet.authCode) {
        		...
        		case QQ.QQ_AUTH_REJECT:
                    ...
                    client.fireQQEvent(e);
                    break;
        		case QQ.QQ_AUTH_QUESTION: //answer question
        			e = new QQEvent(packet);
                    e.type = QQEvent.FRIEND_ADD_NEED_ANSWER;
                    client.fireQQEvent(e);
        			break;
        	}
     ...
     * @param in
     */
    private void processLoginGetListReply(BasicInPacket in) {
        QQEvent e;
        LoginGetListReplyPacket packet = (LoginGetListReplyPacket)in;
        // 触发事件
        e = new QQEvent(packet);
        e.type = QQEvent.LOGIN_GET_LIST_OK;
        client.fireQQEvent(e);            
    }
