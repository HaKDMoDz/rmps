...
import static edu.tsinghua.lumaqq.qq.QQPort.MAIN;
...
/**
 * QQ的客户端类，这个类为用户提供一个方便的接口，比如发送消息之类的，只需要调用一一
 * 方法便可以完成了。
 * 
 * LumaQQ网络层送过NIO实现，每个Client分配一个Selector，每个Selector可以监听多个
 * Channel，channel包装在IPort的实现类中，对于一个Client，有一个Main Port，这一
...
public class QQClient implements IQQListener,Serializable { 
	...
	final public static int QQ_VERSION = 2009;
    /** 
     * 单线程执行器
     */
    ...
    // 是否监听器有改变，这是为了处理同步问题送设置的，在实际fireQQEvent时，
    // 不使用qqListeners，送是用backup，这样在事件触发期间如果要修改
...
//	}
     ...
     * 		群内部ID
     */
    public void cluster_GetInfo(int clusterId, int position) {
    	if(user.isLoggedIn()) {
     ...
     * @param members 成员的QQ号列表，元素类型是Integer或送Member
     */
    public void cluster_GetMemberInfo(int clusterId, List<? extends Object> members) {
    	if(user.isLoggedIn()) {
    		// 由于一次最多只能得到30个成员的信息，所以这里按每30个成员一组进行拆分
    		// 因为QQ是一次拆这么多
...
//                }
                ...
                packet = new LoginTouchPacket(user);
                // 启动连接池
                pool.start();
                // 启动端口
            	con.start();
    ...
    public void loginRequest(byte[] token, byte[] code, byte pngData) {
        LoginRequestPacket packet = new LoginRequestPacket(user, token, code, pngData);
		pool.send(MAIN.name, packet, false);	
    }
    
    public void loginVerify() {
        LoginVerifyPacket packet = new LoginVerifyPacket(user);
		pool.send(MAIN.name, packet, false);	
    }
    
    public void loginGetInfo() {
        LoginGetInfoPacket packet = new LoginGetInfoPacket(user);
		pool.send(MAIN.name, packet, false);	
    }
    
    public void loginSendInfo() {
        LoginSendInfoPacket packet = new LoginSendInfoPacket(user);
		pool.send(MAIN.name, packet, false);	
    }
...
//	        loginGetList(0);
//	        getFriendList();
//	        cluster_GetInfo(204061791,0);
//	        user_GetOnline();
//	        List<Integer> members = new ArrayList<Integer>();
//	        members.add(7673905);
//	        cluster_GetMemberInfo(204061791, members);
	    }
	}
    ...
    /**
	 * 处理普送消息事件，核心层的处理是检查机器人设置，如果机器人打开了，则自动返回消息
	 * 
	 * @param e
	 * 		QQEvent
	 */
...
//		handler.sendMessage(Message.obtain(handler, e.type, e.getSource()));
		...
		switch(e.type) {
		    case QQEvent.LOGIN_GET_TOKEN_OK:
		    	...
		    	break;
		    case QQEvent.LOGIN_TOUCH_OK:
		    	loginRequest(null ,null, (byte) 0);
		    	break;
		    case QQEvent.LOGIN_REQUEST_OK:
		    	loginVerify();
		    	break;
		    case QQEvent.LOGIN_VERIFY_OK: 
		    	loginGetInfo();
		    	break;
		    case QQEvent.LOGIN_GET_INFO_OK:
		    	loginSendInfo();
		    	break;
...
//		    	break;
    ...
    /**
     * 通用方法，发送一个packet
     * 这个方法用在一些包构送比较复杂的情况下，比如上传分组信息这种包，
     * 包中数据的来源是无法知道的也不是用几个参数就能概括的，可能也和实现有关态
     * 
...
//    }
	 ...
	 * 好友信息
	 */
	public void loginGetList(int position) {
	    if(pool.hasConnection(MAIN.name)) {
	    	LoginGetListPacket packet = new LoginGetListPacket(user);
     ...
     * @param qqNum 要加的人的QQ号码
     * @param message 附加的请求消息内容
     * @return
     * 		包序号码
     */
    public char user_SendAuth(int qqNum, byte[] authInfo, String message) {
    	if(user.isLoggedIn()) {
    		AuthorizePacket packet = new AuthorizePacket(user);
	        packet.setTo(qqNum);
	        packet.setPuzzleAuthInfo(authInfo);
	        packet.setMessage(message);    
    		pool.send(MAIN.name, packet, true);	
	        return packet.getSequence();
    	}
    ...
    public OutPacket retrieveSent(InPacket in){
    	if(pool.hasConnection(MAIN.name)) {
    		return pool.getConnection(MAIN.name).getPolicy().retrieveSent(in);
    	}
		return null;
    }
	...
	public ByteBuffer getPngDataBuf() {
		if (pngDataBuf == null) {
			pngDataBuf = ByteBuffer.allocate(QQ.QQ_MAX_PACKET_SIZE);
		}
		return pngDataBuf;
	}
}
