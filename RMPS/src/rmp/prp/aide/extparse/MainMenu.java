 ...
 * CreateDate:     2007-7-18 下午08:20:23
 * ProjectName:    RMPS
...
import rmp.prp.aide.extparse.d.DX0008;
...
public class MainMenu implements WBackCall
{
    /** 父应用引用 */
     ...
     * 
     */
    public MainMenu(Extparse owner)
    {
     ...
     * @return
     */
    public static MainMenu getInstance(Extparse owner)
    {
        if (rm_RmpsMenu == null)
        {
            rm_RmpsMenu = new MainMenu(owner);
            rm_RmpsMenu.wInit();
        }
        return rm_RmpsMenu;
    }
    ...
    private boolean initOlSearch()
    {
        ...
        try
        {
            ...
            for (int i = 0, j = linkList.size(); i < j; i += 1)
            {
                ...
                // 保存链接地址
                menuItem.putClientProperty(ConstUI.PROP_SRCHURLS, kvItem.getK());
                // 注册事件侦听器
    ...
    private boolean initDataExpt()
    {
        ...
        for (int i = 0, j = exptList.length; i < j; i += 1)
        {
            ...
            // 文件路径标记设置
            menuItem.putClientProperty(ConstUI.PROP_DATAEXPT, exptList[i].getPath());
            // 注册事件侦听器
            menuItem.addActionListener(al);
    ...
    private boolean initLangList()
    {
        ...
        try
        {
            ...
            for (int i = 0, j = langList.size(); i < j; i += 1)
            {
                ...
                // 保存语言标记
                menuItem.putClientProperty(ConstUI.PROP_LANGINDX, kvItem.getK());
                // 注册事件侦听器
                menuItem.addActionListener(al);
    ...
    private boolean initSkinList()
    {
        ...
        javax.swing.JCheckBoxMenuItem menuItem;
        // 默认风格
        String[] txt = {LangRes.MENU_TEXT_SKINSYSTEM, LangRes.MENU_TEXT_SKINMETAL, LangRes.MENU_TEXT_SKINMOTIF,
            LangRes.MENU_TEXT_SKINWINXP, LangRes.MENU_TEXT_SKINWINME};
        String[] tip = {LangRes.MENU_TIPS_SKINSYSTEM, LangRes.MENU_TIPS_SKINMETAL, LangRes.MENU_TIPS_SKINMOTIF,
            LangRes.MENU_TIPS_SKINWINXP, LangRes.MENU_TIPS_SKINWINME};
        String[] key = {ISkin.LF_TYPE_SYSTEM, ISkin.LF_NAME_METAL, ISkin.LF_NAME_MOTIF, ISkin.LF_NAME_WINDOWSXP,
            ISkin.LF_NAME_WINDOWSME};
        for (int i = 0, j = txt.length; i < j; i += 1)
        {
            menuItem = createCheckBoxMenuItem(txt[i], tip[i], true);
            menuItem.putClientProperty(ConstUI.PROP_SKINNAME, key[i]);
            menuItem.addActionListener(al);
        ...
        for (int i = 0, j = skinList.length; i < j; i += 1)
        {
            ...
            // 皮肤标记设置
            menuItem.putClientProperty(ConstUI.PROP_SKINNAME, ISkin.LF_TYPE_SYNTH);
            // 注册事件侦听器
            menuItem.addActionListener(al);
    ...
    private boolean initViewForm()
    {
        ...
        // 菜单显示
        String[] txt = {LangRes.MENU_TEXT_DESPFORM, LangRes.MENU_TEXT_MIMEFORM, LangRes.MENU_TEXT_CORPFORM,
            LangRes.MENU_TEXT_SOFTFORM, LangRes.MENU_TEXT_FILEFORM, LangRes.MENU_TEXT_ASOCFORM,
            LangRes.MENU_TEXT_IDIOFORM};
        String[] tip = {LangRes.MENU_TIPS_DESPFORM, LangRes.MENU_TIPS_MIMEFORM, LangRes.MENU_TIPS_CORPFORM,
            LangRes.MENU_TIPS_SOFTFORM, LangRes.MENU_TIPS_FILEFORM, LangRes.MENU_TIPS_ASOCFORM,
            LangRes.MENU_TIPS_IDIOFORM};
        String[] key = {ConstUI.PROP_DESPINFO, ConstUI.PROP_MIMEINFO, ConstUI.PROP_CORPINFO, ConstUI.PROP_SOFTINFO,
            ConstUI.PROP_FILEINFO, ConstUI.PROP_ASOCINFO, ConstUI.PROP_IDIOINFO};
        ...
        for (int i = 0, j = txt.length; i < j; i += 1)
        {
            ...
            menuItem.addActionListener(al);
            menuItem.putClientProperty(ConstUI.PROP_VIEWFORM, key[i]);
    ...
    private void mi_ChckUpdt_Handler(java.awt.event.ActionEvent evt)
    {
        ...
        {
            LogUtil.exception(exp);
            MesgUtil.showMessageDialog(Extparse.getForm(), Extparse.getMesg(LangRes.MESG_OTHR_0006));
            return;
        }

        // 版本比较
        String nVer = updtProp.getProperty(EnvCons.EXTPARSE_SOFTEDIT, "");
        if (ConstUI.VER_SOFT.compareTo(nVer) < 0)
        {
            MesgUtil.showMessageDialog(Extparse.getForm(), Extparse.getMesg(LangRes.MESG_OTHR_0008));
        }
        else
        {
            MesgUtil.showMessageDialog(Extparse.getForm(), Extparse.getMesg(LangRes.MESG_OTHR_0007));
        }
    }
    ...
    {
        UserData userMeta = me_MainSoft.getMainPanel().getUserData(true);
        if (userMeta != null)
        {
            me_MainSoft.getMainPanel().metaDelete(userMeta.getExtsName(), userMeta.getSoftHash());
        }
    }
    ...
    {
        MesgUtil.showMessageDialog(Extparse.getForm(), "此功能正在完善中，敬请关注。。。");
    }
    ...
    {
        // 数据备份确认
        if (0 == MesgUtil.showConfirmDialog(Extparse.getForm(), Extparse.getMesg(LangRes.MESG_SAFE_0001)))
        {
            try
            {
    ...
    {
        // 数据备份确认
        if (0 == MesgUtil.showConfirmDialog(Extparse.getForm(), Extparse.getMesg(LangRes.MESG_SAFE_0002)))
        {
            try
            {
                EnvUtil.shutdownDataBase();
    ...
    private void mi_ViewForm_Handler(java.awt.event.ActionEvent evt)
    {
        ...
        String formName = (String)mi.getClientProperty(ConstUI.PROP_VIEWFORM);
    ...
    private void mi_DataExpt_Handler(java.awt.event.ActionEvent evt)
    {
        ...
        // 用户输入数据信息取得
        BaseData baseMeta = me_MainSoft.getMainPanel().getBaseMeta();
        if (baseMeta == null)
        {
            return;
        }
        ...
        // 模板文件路径取得
        javax.swing.JMenuItem mi = (javax.swing.JMenuItem)obj;
        File tpltFile = new File((String)mi.getClientProperty(ConstUI.PROP_DATAEXPT));
        if (!tpltFile.exists())
        {
            return;
        }
        ...
        // 导出文件路径取得
        File exptFile = FileUtil.showSingleFileSave(Extparse.getForm(), true, null);
        if (exptFile == null)
        {
            return;
        }
        exptFile = new File(exptFile, baseMeta.getExtsName().substring(1) + FileUtil.getFileExt(tpltFile));

        // 数据导出
        try
        {
            DX0008.export(exptFile, tpltFile, baseMeta, RmpsUtil.getUserInfo().getCfg(CfgCons.CFG_LANG_ID), true);
        }
        catch(IOException e)
        {
            e.printStackTrace();
    ...
    private void mi_LangAll_Handler(java.awt.event.ActionEvent evt)
    {
        ...
        catch(SQLException exp)
        {
            ...
            MesgUtil.showMessageDialog(Extparse.getForm(), msg);
        }
        finally
        {
            dba.closeConnection();
        ...
        if (langList != null)
        {
            // WListForm.showDialog(Extparse.getForm(), LangRes.TITLE_CORPFORM,
            // langList, this);
        }
    }
    ...
    private void mi_LangEdit_Handler(java.awt.event.ActionEvent evt)
    {
        ...
        langEdit.wInit();
        langEdit.wShowView(Extparse.VIEW_NORM);
    }
    ...
    private void mi_LangItem_Handler(java.awt.event.ActionEvent evt)
    {
        ...
        String langHash = (String)mi.getClientProperty(ConstUI.PROP_LANGINDX);
    ...
    private void mi_SkinItem_Handler(java.awt.event.ActionEvent evt)
    {
        ...
        String skinName = (String)cmi.getClientProperty(ConstUI.PROP_SKINNAME);
        ...
        javax.swing.SwingUtilities.updateComponentTreeUI(Extparse.getForm());
        javax.swing.SwingUtilities.updateComponentTreeUI(pm_MainMenu);
        // Extparse.getForm().pack();
        ...
        // 提示用户重新启动应用程序
        MesgUtil.showMessageDialog(Extparse.getForm(), Extparse.getMesg(LangRes.MESG_OTHR_0001));
    }
    ...
    private void mi_SrchItem_Handler(java.awt.event.ActionEvent evt)
    {
        ...
        // 用户数据取得
        UserData userMeta = me_MainSoft.getMainPanel().getUserData(false);
        if (userMeta == null)
        {
            return;
        }
    ...
    // ////////////////////////////////////////////////////////////////////////
    /** 系统菜单 */
    private static MainMenu        rm_RmpsMenu;
    // ----------------------------------------------------
    // 系统子菜单
    // ----------------------------------------------------
    /** 数据编辑 */
