...
public class LoginGetListPacket extends BasicOutPacket {
     ...
     * 构造函数
     */
    public LoginGetListPacket(QQUser user) {
        super(QQ.QQ09_CMD_LOGIN_GET_LIST, true, user);
        position = 0;
    }
     ...
     */
    public LoginGetListPacket(ByteBuffer buf, int length, QQUser user) throws PacketParseException {
    ...
    protected void putBody(ByteBuffer buf) {
		...
		buf.putChar((char) position );
		buf.putChar((char) 0x0000 );
		buf.putChar((char) 0x0071 );
		for (int i=0; i<0x0071; i++) {//0x0071 zeros
			buf.put((byte)0);
		}
    }
