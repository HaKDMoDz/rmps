...
//                	return new GetLoginTokenReplyPacket(buf, len, user);
                	return new LoginRequestReplyPacket(buf, len, user);
                case QQ.QQ09_CMD_LOGIN_TOUCH:
                	return new LoginTouchReplyPacket(buf, len, user);
                case QQ.QQ09_CMD_LOGIN_VERIFY:
                	return new LoginVerifyReplyPacket(buf, len, user);
                case QQ.QQ09_CMD_LOGIN_GET_INFO:
                	return new LoginGetInfoReplyPacket(buf, len, user);
                case QQ.QQ09_CMD_LOGIN_SEND_INFO:
                	return new LoginSendInfoReplyPacket(buf, len, user);
                case QQ.QQ09_CMD_LOGIN_GET_LIST:
                	return new LoginGetListReplyPacket(buf, len, user);
	...
	public OutPacket parseOutcoming(ByteBuffer buf, int len, QQUser user) throws PacketParseException {
	    try {
	    	char command = getCommand(buf, user);
            switch(command) {
                case QQ.QQ09_CMD_LOGIN_REQUEST:
//                	return new GetLoginTokenPacket(buf, len, user);
                	return new LoginRequestPacket(buf, len, user);
                case QQ.QQ09_CMD_LOGIN_TOUCH:
                	return new LoginTouchPacket(buf, len, user);
                case QQ.QQ09_CMD_LOGIN_VERIFY:
                	return new LoginVerifyPacket(buf, len, user);
                case QQ.QQ09_CMD_LOGIN_GET_INFO:
                	return new LoginGetInfoPacket(buf, len, user);
                case QQ.QQ09_CMD_LOGIN_SEND_INFO:
                	return new LoginSendInfoPacket(buf, len, user);
                case QQ.QQ09_CMD_LOGIN_GET_LIST:
                	return new LoginGetListPacket(buf, len, user);
            	case QQ.QQ_CMD_KEEP_ALIVE:
            	...
            	case QQ.QQ_CMD_RECV_IM:
            	case QQ.QQ_CMD_RECV_IM_09:
            		ReceiveIMReplyPacket packet = new ReceiveIMReplyPacket(buf, len, user);
